...
public class DynamicLight : MonoBehaviour {
	...
	void setLight () {
		...
		//for (int m = 0; m < 1; m++) {
			...
			for (int i = 0; i < mf.GetTotalPointCount(); i++) {								   // ...and for ever vertex we have of each mesh filter...
				...
				Debug.DrawLine(transform.position, v.pos, v.endpoint ? Color.red : Color.white);	
		...
		for (int i = 0; i < lightSegments; i++)  {
			...
			v.pos = new Vector3((FastMath.SinArray[theta]), (FastMath.CosArray[theta]), 0); // in dregrees (previous calculate)
	...
	void renderLightMesh(){
        ...
        var meshVertices = new Vector3[allVertices.Count + 1];
        var meshUvs = new Vector2[meshVertices.Length];
        var meshTriangles = new int[(allVertices.Count * 3)];
        ...
        {
            meshVertices[i + 1] = allVertices[i].pos;
        }
        ...
        // triangles
        for (int i = 0, j = 0, len = allVertices.Count * 3; i < len; i += 3, j++)
        {
